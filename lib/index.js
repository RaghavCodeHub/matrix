 ...
 *
 * @param mat input matrix that initialized the function
 * @returns adjunct matrix
 */

 function adjunct(mat) {
    const [rows, cols] = size(mat)

    const submtx = [] //variable to store the sub matrixes
    const confactors = [] //variable to store the cofactors array

    let index = () => { //function to return the indexs
        let idx = []
    ...
    let index_cols = () => {
        const ic = index()
        const [icRows, icCols] = size(ic)
        let i = 0
        while (i < icRows) {
            for (let c = 0; c < icCols; c++) {
                ic[i].splice(c, 1)
                i++
            }
        }
    ...
    //get the cofactor of submtx
    let n = 0
    for (let l = 0; l < rows; l++) {
        for (let c = 0; c < cols; c++) {
            confactors.push(cofactor(l + 1, c + 1, submtx[n]))
            n++
        }
    }

    //transform the confactors array in a matrix
    let mtx_confactors = []
    for (let i = 0; i < rows; i++) {
        mtx_confactors.push(confactors.slice(0, cols))
        confactors.splice(0, cols)
    }

    return trans(mtx_confactors)
}

function cofactor(rows, cols, submtx) {
    return (-1) ** (rows + cols) * determinant(submtx)
}
